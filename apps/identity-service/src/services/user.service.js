...
export const loginUserService = async (email, password, context) => {
  ...
  if (user.isTwoFactorEnabled) {
    ...
    await VerificationToken.deleteMany({ userId: user._id, type: "2fa_challenge" });
    ...
    logger.info(`[2FA_REQUIRED] RID: ${requestId} - DB-backed OTP sent to: ${email}`);
...
export const verifyTwoFAService = async (otpToken, otpCode, context) => {
  ...
  try {
    ...
    throw new ErrorHandler("Invalid or expired session", HTTP_STATUS.UNAUTHORIZED);
  }
  ...
  if (!tokenDoc) {
    logger.warn(`[2FA_VERIFY_FAILED] RID: ${requestId} - Invalid OTP for User: ${decoded.userId}`);
    throw new ErrorHandler("Invalid or expired OTP code", HTTP_STATUS.BAD_REQUEST);
  }
  ...
  logger.info(`[2FA_VERIFY_SUCCESS] RID: ${requestId} - User: ${user.email} session: ${session._id}`);
